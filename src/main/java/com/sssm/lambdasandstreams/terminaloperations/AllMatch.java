...
public class AllMatch {
	...
	public static void main(String[] args) {
		...
		boolean answer = list.stream().allMatch(n -> (n % 2 == 0));
		System.out.println(answer);
		
		list = Arrays.asList(4, 6, 12, 20);
		// Stream allMatch(Predicate predicate)
		answer = list.stream().allMatch(n -> (n % 2 == 0));
		System.out.println(answer);
		...
		answer = doubleStream.allMatch(num -> (num - 5) > 6);
		System.out.println(answer);

		// IntStream anyMatch example
		IntStream intStream = IntStream.of(1, 2, 3, 4, 5, 6);
		// Stream anyMatch(Predicate predicate)
		answer = intStream.allMatch(num -> (num - 1) >= 0);
		System.out.println(answer);
